...
const VerifyEmail: React.FC = () => {
    ...
    const handleConfirm = async (): Promise<void> => {
        try {
        const response = await axios.post<{ message: string }>(`${API_BASE_URL}/auth/verify-email`, {
            token: token,
          });
  ...
  return (
    <div style={{
        ...
        <p>Please click the button below to complete the email verification and finish the registration.</p>
        <button 
        style={{    
            marginTop: '20vw',
            width: '80vw',
            ...
            fontWeight: 'bold',
            background: '#624a92'}}
        onClick={handleConfirm}>Validate</button>
        {message && <p>{message}</p>}
        </div>
    </div>
  );
