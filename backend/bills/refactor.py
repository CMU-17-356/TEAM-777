...
def get_member_balances(db, group_id, user_id):
    try:
        ...
        if not ObjectId.is_valid(group_id):
            return {}
        transactions = list(db.transactions.find({"groupId": ObjectId(group_id)})) if hasattr(db.transactions, 'find') else []
        group = db.groups.find_one({"_id": ObjectId(group_id)}) if hasattr(db.groups, 'find_one') else None
        if not group or 'members' not in group or not isinstance(group['members'], list):
            return {}
        # Convert all member IDs to strings
        member_ids = [str(member_id) for member_id in group["members"] if str(member_id) != str(user_id)]
        member_balances = {member_id: 0.0 for member_id in member_ids}
        for transaction in transactions:
            if not all(k in transaction for k in ("type", "initiatorId", "recipientId", "amount")):
                continue
            ...
            if transaction["type"] == "request":
                if recipient_id == str(user_id):
                    member_balances[initiator_id] = member_balances.get(initiator_id, 0.0) + float(transaction["amount"])
                elif initiator_id == str(user_id):
                    member_balances[recipient_id] = member_balances.get(recipient_id, 0.0) - float(transaction["amount"])
...
def create_transaction(db):
    try:
        ...
        required_fields = ['group_id', 'initiator_id', 'recipient_id', 'amount', 'description', 'transaction_type']
        if not all(field in data for field in required_fields):
            return jsonify({"success": False, "message": "Missing required fields"}), 400
        if not hasattr(db, 'groups') or not hasattr(db, 'transactions'):
            return jsonify({"success": False, "message": "Database not properly configured"}), 500
        if not ObjectId.is_valid(data['group_id']):
            return jsonify({"success": False, "message": "Invalid group_id"}), 400
        group = db.groups.find_one({"_id": ObjectId(data['group_id'])}) if hasattr(db.groups, 'find_one') else None
        if not group or 'members' not in group or not isinstance(group['members'], list):
            return jsonify({"success": False, "message": "Group not found or has no members"}), 404
        group_member_ids = [str(mid) for mid in group["members"]]
        if str(data['initiator_id']) not in group_member_ids or str(data['recipient_id']) not in group_member_ids:
            return jsonify({"success": False, "message": "Users must be group members"}), 400
        ...
        result = db.transactions.insert_one(transaction) if hasattr(db.transactions, 'insert_one') else None
...
def format_transactions(db, group_id: str) -> List[Dict[str, Any]]:
    ...
    for entry in history.values():
        ...
        splitter_usernames = [
            get_user_info(db, uid)["username"] for uid in splitters if ObjectId.is_valid(uid)
        ]
...
def handle_add_expense(db):
    ...
    try:
        ...
        if not isinstance(data["splitters"], list) or not data["splitters"]:
            return jsonify({"success": False, "message": "Splitters must be a list"}), 400
        ...
        record = collection.find_one({"_id": group_id}) or {"_id": group_id, "history": {}}
